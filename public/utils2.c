...
int bufferEncrypt(char *buf, int size, char *target, int targetSize, char *key, int keySize)
{
	...
	ret=lzo1x_1_compress(buf, size, target, &retSize, tmp);
	if(ret==LZO_E_OK)
	{
...
int bufferDecrypt(char *buf, int size, char *target, int targetSize, char *key, int keySize)
{
	...
	xorEncode2(buf, size);
	ret=lzo1x_decompress_safe(buf, size, target, &retSize, NULL);
	if(ret==LZO_E_OK)
	{
...
char *loadFileMem(const char *fname, int *fsize, int encrypt, const char *key, int keySize)
{
	int flen;
	FILE *f;
	char *buffer;
...
#endif
//zsliu add 2009-01-04
#include <sys/statvfs.h>
#include <sys/stat.h>
#ifndef HTTPD_SDK
long getFreeSpace(const char *fs)
{
	struct statvfs buffer;
	int            status;
	status = statvfs(fs, &buffer);
/*	printf("bsize=%d, rsize=%d, blocks=%d, bfree=%d, bavailable=%d, ffree=%d
			, favialible=%d\n",
			buffer.f_bsize, buffer.f_frsize,
			buffer.f_blocks, buffer.f_bfree, buffer.f_bavail,
			buffer.f_ffree, buffer.f_favail);*/
	if(status>=0)
...
int isFileSymbolLink(const char *fileName)
{
	struct stat buffer;
	int         status;
	status = lstat(fileName, &buffer);
	if(status<0) return status;
//	printf("file=%s, mode=%X, size=%d, nlink=%d\n", fs,
//			buffer.st_mode, buffer.st_size, buffer.st_nlink);
	return S_ISLNK(buffer.st_mode);
}

int delFileHead(int fd, int size)
{
	int tail=size, head=0;
	const int block=64*1024;
	char buf[64*1024];
	printf("delFileHead() truncate before %d\n", size);
	while(1)
	{
		if(lseek(fd, tail, SEEK_SET)==tail)
		{
			int len=read(fd, buf, block);
			if(len>0)
			{
			...
			if(len<block)
			{
				printf("delFileHead() READ OVER head=%d, tail=%d, len=%d\n", head, tail, len);
			       	break;
			}
		}
		else
		{
			printf("delFileHead() SEEK TAIL ERROR: head=%d, tail=%d\n", head, tail);
			break;
		}
	}
	ftruncate(fd, head);
	printf("delFileHead() truncate from %d\n", lseek(fd, 0, SEEK_CUR));
	return 0;
}
//add end 2009-01-04
