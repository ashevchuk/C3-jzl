...
{
	PStrList old, strLst=slCreate("");
#define	fname "G:\\tools\\tftpd\\x.txt"
	...
	{
		int i, len;
		char *itemValue, *itemName;
		slSetLines(strLst, mem);
		printf("System.UpdateFw=%s\n%s",slGetValue(strLst,"System.UpdateFw"), slGetValue(strLst,"sjkjsd"));
		for(i=0;i<strLst->count;i++)	
		{
			char item[100] = {0};
			len = 100;
			itemValue=slNameValueAt(strLst, i, item, &len);	
			itemName = item;
			printf("%s=%s\n",itemName, itemValue);
		}
		slSaveToFile(strLst, fname".txt");
	}
	slFree(strLst);
}
